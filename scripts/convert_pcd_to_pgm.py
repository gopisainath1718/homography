...
grid_for_pgm = np.where(cropped_grid == 100, 0,    # Occupied -> black
                        np.where(cropped_grid == -1, 180,  # Unknown -> gray
                                 205))                     # Free -> light gray
print("Unique values in grid_for_pgm:", np.unique(grid_for_pgm))

# Save the grid as a PGM image
img = Image.fromarray(np.uint8(grid_for_pgm))
img.save("map.pgm")

# Adjust origin for cropping
cropped_min_x = min_x * resolution + min_x
cropped_min_y = min_y * resolution + min_y

# Create the YAML file
yaml_data = {
    "image": "map.pgm",
    "resolution": resolution,
    "origin": [float(cropped_min_x), float(cropped_min_y), 0.0],
    "negate": 0,
    "occupied_thresh": 0.65,  # Slightly higher for AMCL to be more conservative
    "free_thresh": 0.196
}
...
print("Conversion successful. map.pgm and map.yaml have been generated.")
